...
export default function LifestylePage() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 md:px-6">
      <h1 className="text-3xl font-bold text-slate-900">Lifestyle tips</h1>
      ...
      <ul className="mt-8 flex flex-wrap gap-2">
        {sections.map((s) => (
          <li key={s.id}>
            <a
              href={`#${s.id}`}
              className="inline-flex rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 hover:border-brand-primary/40 hover:text-brand-primary"
            >
              {s.title}
            </a>
          </li>
