...
import { SLEEP_RELATED } from "@/lib/lifestyle/sleep-content";

export function SleepRelatedGuides() {
  return (
    <section>
          ...
          {SLEEP_RELATED.map((card) => (
            ...
            >
              <div className="flex flex-wrap items-center gap-2 text-xs font-medium uppercase tracking-wide text-slate-500">
                <span className="text-indigo-800/90">{card.category}</span>
              ...
              <Link
                href={card.href}
                className="group mt-6 inline-flex items-center gap-1 text-sm font-semibold text-indigo-800 transition hover:text-indigo-950"
              >
                Read more
