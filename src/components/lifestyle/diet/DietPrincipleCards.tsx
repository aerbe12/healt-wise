...
import { Beef, LeafyGreen, Droplets, Factory } from "lucide-react";
import { DIET_PRINCIPLES } from "@/lib/lifestyle/diet-content";

const ICONS: LucideIcon[] = [Beef, LeafyGreen, Droplets, Factory];

export function DietPrincipleCards() {
  return (
    <section>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <h2 className="max-w-2xl text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
          Key principles of an effective diet
        </h2>
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
          {DIET_PRINCIPLES.map((item, i) => {
            const Icon = ICONS[i] ?? LeafyGreen;
            return (
              <article
                key={item.title}
