...
import {
  averagePriceAcrossDosesMounjaro,
...
type Props = { providers: MounjaroUkProviderCompare[] };

export function MounjaroUkPriceRangeByDoseChart({ providers }: Props) {
  const data = dosePriceRangeByStrengthMounjaro(providers).map((r) => ({
    ...
    <figure className="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm md:p-6">
      <figcaption className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Min and max pen prices across listed providers (same snapshot). Purple
...
export function MounjaroUkDosageCostLineChart({ providers }: Props) {
  const data = averagePriceAcrossDosesMounjaro(providers).map((r) => ({
    dose: r.dose.replace("mg", " mg"),
    avg: r.avg,
  }));
