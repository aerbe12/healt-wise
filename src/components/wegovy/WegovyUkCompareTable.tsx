...
export default function WegovyUkCompareTable({
  ...
  const processed = useMemo(() => {
    ...
    rows = [...rows].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "updated":
          ...
          break;
        default:
          if (WEGOVY_DOSE_KEYS.includes(sortKey as WegovyDoseColumnKey)) {
            cmp =
              a.prices[sortKey as WegovyDoseColumnKey] -
              b.prices[sortKey as WegovyDoseColumnKey];
          }
      }
      return cmp * dir;
    });
    ...
    for (const k of WEGOVY_DOSE_KEYS) {
      if (rows.length > 0) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-xl border border-slate-200/80 bg-slate-50/90 px-4 py-3 text-xs text-slate-600">
        ...
        <span>
          Private Wegovy routes use an online consultation; confirm the supplying
          pharmacy on the{" "}
          <a
