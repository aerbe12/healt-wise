...
type WegovyTableSort =
  | { key: "composite"; dir: "asc" | "desc" }
  | { key: "dose"; dose: WegovyDoseColumnKey; dir: "asc" | "desc" }
  | { key: "rating"; dir: "asc" | "desc" }
  | { key: "provider"; dir: "asc" | "desc" };

function wegovySortToPreset(s: WegovyTableSort): CompareSortPreset {
  switch (s.key) {
    case "composite":
    case "dose":
...
function presetToWegovySort(v: CompareSortPreset): WegovyTableSort {
  switch (v) {
    case "price-asc":
...
export default function WegovyUkCompareTable({
  ...
  const [tableSort, setTableSort] = useState<WegovyTableSort>({
    key: "provider",
    dir: "asc",
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <CompareFilterBar
        ...
        sort={wegovySortToPreset(tableSort)}
        onSortChange={(v) => setTableSort(presetToWegovySort(v))}
        pillRegionTop={
          <>
        ...
        filterExtras={
          <>
            <p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-emerald-900">
              Price band
            </p>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm">
        ...
        <div className="max-lg:max-h-none max-lg:min-h-0 max-lg:w-full max-lg:min-w-0 max-lg:overflow-x-auto max-lg:overflow-y-visible max-lg:overscroll-x-contain max-lg:touch-pan-x lg:max-h-[min(78vh,800px)] lg:overflow-auto lg:overscroll-contain min-h-0 w-full min-w-0 [overflow-anchor:none]">
