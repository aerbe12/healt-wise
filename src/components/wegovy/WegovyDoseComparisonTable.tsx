          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-slate-600">
                ...
                <span>
                  {row.rating.toFixed(1)} ({row.reviewCount.toLocaleString("en-GB")})
                </span>
              </div>
              {row.promo ? (
                <p className="mt-2 text-xs font-medium text-emerald-700">{row.promo}</p>
              ) : null}
            </div>
          </div>
          <dl className="mt-4 grid grid-cols-2 gap-2 text-sm">
            <div>
              <dt className="text-slate-500">Price ({WEGOVY_DOSE_LABEL[dose]})</dt>
              <dd className="font-semibold text-slate-900">{formatGBP(row.priceGbp)}</dd>
            </div>
            <div>
            ...
            {row.repeatPriceGbp != null ? (
              <div className="col-span-2">
                <dt className="text-slate-500">Repeat / maintenance (if stated)</dt>
                <dd className="font-medium text-slate-800">
      ...
      <table className="w-full min-w-[720px] text-left text-sm">
        <thead>
          <tr className="border-b border-brand-border bg-slate-50/90">
            ...
            <th className="px-4 py-3 font-semibold text-slate-800">Repeat price</th>
            <th className="px-4 py-3 font-semibold text-slate-800">Delivery</th>
            <th className="px-4 py-3 font-semibold text-slate-800">Trust &amp; safety</th>
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.id} className="border-b border-slate-100 last:border-0">
              <td className="px-4 py-4 align-top">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {row.promo ? (
                      <p className="mt-1 text-xs font-medium text-emerald-700">{row.promo}</p>
                    ) : null}
                  </div>
                </div>
              </td>
              ...
              <td className="px-4 py-4 align-top text-slate-700">
                {row.repeatPriceGbp != null ? formatGBP(row.repeatPriceGbp) : "—"}
              </td>
  ...
  return (
    <section id={id} className="scroll-mt-24">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Illustrative snapshots — last updated {WEGOVY_PRICE_LAST_UPDATED}. Confirm on the
            provider site before you pay.
          </p>
        </div>
        <p className="text-xs text-slate-500">
          UK private pricing often falls roughly between{" "}
          <strong className="text-slate-700">£169–£269</strong> per order at common maintenance
          doses in our sample; wider market tiers exist.
        </p>
      </div>
      ...
      <div className="mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:justify-center">
        ...
        >
          Compare cheapest Wegovy providers (full hub)
        </Link>
      </div>
    </section>
  );
